...
namespace lsm {
  class ManifestManager {
    public: 
      ...
      void remove_sstable(const SSTableMetadata& metadata);
      std::vector<std::vector<std::shared_ptr<SSTableMetadata>>> load();

    private: 
      std::filesystem::path work_dir_;
      std::unique_ptr<std::ofstream> active_manifest_;

      std::vector<std::vector<std::shared_ptr<SSTableMetadata>>> recover_(const std::string& name);
